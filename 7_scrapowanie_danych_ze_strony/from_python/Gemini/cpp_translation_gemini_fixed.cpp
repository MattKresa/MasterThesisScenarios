...
GumboNode* findElementByClass(GumboNode* node, const std::string& class_name) {
    ...
    if (node->v.element.tag == GUMBO_TAG_DIV || node->v.element.tag == GUMBO_TAG_SPAN || node->v.element.tag == GUMBO_TAG_SMALL || node->v.element.tag == GUMBO_TAG_A || node->v.element.tag == GUMBO_TAG_LI) {
        GumboAttribute* class_attr = gumbo_get_attribute(&node->v.element.attributes, "class");
        if (class_attr && class_attr->value == class_name) {
            return node;
        }
    ...
    for (unsigned int i = 0; i < children->length; ++i) {
        ...
        if (child->type == GUMBO_NODE_ELEMENT) { // Only proceed if the child is an element
...
std::string getNextPage(GumboNode* root) {
    ...
    if (next_li) {
        const GumboVector* children = &next_li->v.element.children;
        for (unsigned int i = 0; i < children->length; ++i) {
            GumboNode* child = static_cast<GumboNode*>(children->data[i]);
