...
public class SystemSolverFixed {
    ...
    public static Object[] readSystem(String filename) throws IOException {
        ...
        for (String rawLine : lines) {
            ...
            if (line.contains("=") && !line.contains("'") && !line.contains("y0")) {
                String[] parts = line.split("=", 2);
                String key = parts[0].trim();
                String val = parts[1].trim();
                try {
                    Equation eq = new Equation(key, val);
                    Expression e = new ExpressionBuilder(eq.expr).build();
                    params.put(key, e.evaluate());
